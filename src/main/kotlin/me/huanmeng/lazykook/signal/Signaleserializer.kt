...
import me.huanmeng.lazykook.signal.event.SignalEvent
 ...
 * @author huanmeng_qwq
 */
class SignalEventDeserializer : JsonDeserializer<SignalEvent>() {
    override fun deserialize(p: JsonParser, ctxt: DeserializationContext): SignalEvent {
        ...
        return SignalEvent(channelType, type, targetId, authorId, content, msgId, msgTimestamp, nonce, extra)
    }
}
...
class SignalExtraDeserializer : JsonDeserializer<Any>() {
    override fun deserialize(p: JsonParser, ctxt: DeserializationContext): Any? {
        ...
        if (node.has("channel_type")) {
            return ctxt.readTreeAsValue(node, SignalEvent::class.java)
        }
